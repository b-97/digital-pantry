...
function renderHomePage() {
	...
	html += "<button onclick=\"requestPageContent('/ingredients_quantity_page', 'page-body', null)\">Add a Recipe</button>";
	return html;
}
...
function renderIngredientsQuantityPage() {
	...
	html += "<button onclick=\"requestPageContent('/create_recipe_page', 'page-body', {quantity: $('#ingredients_quantity').val(), user_name: data_user_name})\">Submit</button>";
	return html;
}
...
function renderCreateRecipePage(ingredients_quantity, rows) {
	...
	for (var i = 0; i < ingredients_quantity; i++) {
		...
		html += "<input type='text' id='text_ingredient_" + i + "_quantity' placeholder='Enter quantity'>";
		html += "</div>";
	}
	html += "</fieldset>";
